-- Require a real card_proof attachment from the seller before inventory confirmation.
-- This replaces the temporary rollout compatibility that accepted attachment_type = 'other'.
...
begin
  ...
  update public.orders set
    subtotal_crc=v_subtotal,
    total_crc=v_subtotal+coalesce(shipping_fee_crc,0),
    inventory_changes_pending=v_changed,
    response_required_from=case when v_changed then 'buyer' else 'seller' end,
    response_required_at=now(),
    status=case when v_changed then 'inventory_confirmation' else 'preparing_shipment' end,
  ...
  values(p_order_id,v_user,'seller',
    case when v_changed then 'inventory_changes_proposed' else 'inventory_confirmed' end,
...
'Order flow v3: seller must upload attachment_type card_proof before inventory confirmation or quantity-change proposal.';
