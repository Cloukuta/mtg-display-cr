-- Checkout v2: structured cart validation for stale carts and partial reservations.
-- The cart remains keyed by inventory_item_id; this RPC returns human-readable
-- printing details so the UI can identify exactly which variant changed.

create or replace function public.validate_checkout_cart(
  p_seller_id uuid,
  p_items jsonb
)
returns table (
...
begin
  ...
  with requested as (
    ...
    left join public.cards c on c.id=i.card_id
  )
  select
    x.item_id,
    x.qty,
...
'Checkout v2 preflight. Returns exact inventory-item availability and variant metadata. UI should remove resolution=remove, clamp resolution=reduce, and revalidate again during create_checkout_order.';
