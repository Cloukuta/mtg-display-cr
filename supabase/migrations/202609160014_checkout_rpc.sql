...
-- Prices and stock are resolved server-side; the browser never supplies a trusted price.
...
begin
  ...
  if v_buyer_id = p_seller_id then raise exception 'You cannot buy from your own catalog'; end if;
  ...
  if not found then raise exception 'Seller catalog is unavailable'; end if;
  ...
  if not found then raise exception 'Selected delivery point is not shared by buyer and seller'; end if;
  ...
  values(p_seller_id,v_buyer_id,auth.jwt()->>'email',nullif(v_buyer.public_name,''),nullif(v_buyer.whatsapp,''),nullif(btrim(p_customer_note),''),'inventory_confirmation',v_point.id,v_point.store_name,v_point.location_name,nullif(v_seller.sinpe_mobile,''),now(),now())
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    where id=(v_item->>'inventory_item_id')::bigint and seller_id=p_seller_id and available=true and quantity>=v_qty
    for update;
    if not found then raise exception 'An item is no longer available in the requested quantity'; end if;
    ...
    if v_inventory.pricing_mode='custom' then
      ...
      join public.cards c on c.scryfall_id=cp.scryfall_id
      where c.id=v_inventory.card_id and cp.source='cardkingdom'
        and lower(regexp_replace(btrim(cp.finish),'[ _-]+','','g'))=v_finish
        and upper(btrim(cp.condition))=v_condition
      limit 1;
      if v_price_usd is null then raise exception 'An item no longer has a valid sale price'; end if;
    ...
    if v_unit_price is null then raise exception 'An item no longer has a valid sale price'; end if;
  ...
  values(v_order_id,v_buyer_id,'buyer','order_created','Buyer submitted checkout for seller inventory confirmation');
  return v_order_id;
end;
$$;
