...
export default function AppHeader({currentPath=""}:AppHeaderProps){
 const supabase=getSupabase(),pathname=usePathname();const[menuOpen,setMenuOpen]=useState(false),[profile,setProfile]=useState<SellerProfile|null>(null),[userId,setUserId]=useState<string|null>(null),[authReady,setAuthReady]=useState(false),[pendingSellerActions,setPendingSellerActions]=useState(0);
 ...
 return <><header className="sticky top-0 z-50 border-b border-border bg-background/95 text-foreground shadow-sm backdrop-blur supports-[backdrop-filter]:bg-background/85"><div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5"><BrandLogo/><div className="flex items-center gap-2">{userId&&currentPath==="/pending-sales"&&<PendingSalesTestButton/>}{userId&&currentPath==="/catalog"&&pathname==="/catalog/binders"&&<CatalogInventoryEditor/>}{authReady&&userId&&<NotificationsBell userId={userId}/>}<button type="button" onClick={()=>setMenuOpen(true)} aria-label="Open navigation menu" aria-expanded={menuOpen} className="relative flex h-10 items-center gap-2 rounded-xl border border-border bg-card px-3 text-sm font-semibold text-muted-foreground transition hover:text-foreground"><Menu size={19}/><span className="hidden sm:inline">Menu</span>{userId&&pendingSellerActions>0&&<span className="absolute -right-2 -top-2 grid min-h-5 min-w-5 place-items-center rounded-full bg-amber-500 px-1 text-[10px] font-black leading-none text-black shadow-md">{pendingSellerActions>99?"99+":pendingSellerActions}</span>}</button></div></div></header><AppMenu open={menuOpen} onClose={()=>setMenuOpen(false)} currentPath={currentPath} sellerName={profile?.public_name} sellerSlug={profile?.slug} userId={userId} authReady={authReady}/></>
}
