...
export default function CatalogInventoryEditor(){
 ...
 const[items,setItems]=useState<Item[]>([]),[binders,setBinders]=useState<Binder[]>([]),[index,setIndex]=useState(-1),[notice,setNotice]=useState("");
 ...
 async function load(){if(!supabase)return;setNotice("");const{data:auth}=await supabase.auth.getUser();const user=auth.user;if(!user)return;const[itemResult,binderResult]=await Promise.all([supabase.from("inventory_items").select("id,quantity,condition,language,finish,pricing_mode,custom_price_crc,binder_id,cards(name,set_code,collector_number,image_uri)").eq("seller_id",user.id).gt("quantity",0).order("updated_at",{ascending:false}),supabase.from("binders").select("id,name,is_default,is_public").eq("seller_id",user.id).order("is_default",{ascending:false}).order("name")]);if(itemResult.error)setNotice(itemResult.error.message);else setItems((itemResult.data||[]) as unknown as Item[]);if(binderResult.error)setNotice(binderResult.error.message);else setBinders((binderResult.data||[]) as Binder[])}
 useEffect(()=>{void load()},[supabase]);

 // The catalog already renders the seller's cards. Make each card name the editor trigger
 // without adding a second global "Edit inventory" action to the header.
 useEffect(()=>{
  function identifyHeading(target:EventTarget|null){
   const el=target instanceof Element?target:null;if(!el)return null;
   const heading=el.closest("article h2");if(!(heading instanceof HTMLElement))return null;
   const article=heading.closest("article");if(!article)return null;
   return {heading,article};
  }
  function findIndex(heading:HTMLElement,article:Element){
   const name=(heading.textContent||"").trim();
   const detail=Array.from(article.querySelectorAll("p")).map(p=>p.textContent||"").join(" ");
   let matches=items.map((item,i)=>({item,i})).filter(x=>x.item.cards?.name===name);
   const exact=matches.find(x=>{const c=x.item.cards;if(!c)return false;return detail.includes(c.set_code.toUpperCase())&&detail.includes(`#${c.collector_number}`)});
   return exact?.i??matches[0]?.i??-1;
  }
  function onClick(e:MouseEvent){const found=identifyHeading(e.target);if(!found)return;const i=findIndex(found.heading,found.article);if(i<0)return;e.preventDefault();setIndex(i)}
  function onKey(e:KeyboardEvent){if(e.key!=="Enter"&&e.key!==" ")return;const found=identifyHeading(e.target);if(!found)return;const i=findIndex(found.heading,found.article);if(i<0)return;e.preventDefault();setIndex(i)}
  function decorate(){document.querySelectorAll("article h2").forEach(node=>{if(!(node instanceof HTMLElement))return;node.style.cursor="pointer";node.style.textDecoration="underline";node.style.textDecorationColor="rgba(163,230,53,.45)";node.style.textUnderlineOffset="3px";node.setAttribute("role","button");node.setAttribute("tabindex","0");node.setAttribute("title",es?"Editar esta carta":"Edit this card")})}
  decorate();const observer=new MutationObserver(decorate);observer.observe(document.body,{childList:true,subtree:true});document.addEventListener("click",onClick);document.addEventListener("keydown",onKey);
 ...
 async function save(next:Item){if(!supabase)return;const payload={quantity:next.quantity,condition:next.condition,language:next.language,finish:next.finish,pricing_mode:next.pricing_mode,custom_price_crc:next.pricing_mode==="custom"?next.custom_price_crc:null,binder_id:next.binder_id,available:next.quantity>0,updated_at:new Date().toISOString()};const{error}=await supabase.from("inventory_items").update(payload).eq("id",next.id);if(error){setNotice(error.message);return}setItems(current=>current.map(row=>row.id===next.id?{...row,...next}:row));setNotice(es?"Cambios guardados.":"Changes saved.");setIndex(-1);window.location.reload()}
