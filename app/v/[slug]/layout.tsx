...
export default function SellerDisplayLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    void(async()=>{
      ...
      // Catalog handoff is only an initial storefront search. Once the card name
      // has been resolved, remove exact-printing params so shoppers can freely
      // edit/clear the search and immediately return to the seller's full catalog.
      const cleanUrl=new URL(window.location.href);
      cleanUrl.searchParams.delete("card");
      cleanUrl.searchParams.delete("set");
      cleanUrl.searchParams.delete("collector");
      window.history.replaceState(window.history.state,"",`${cleanUrl.pathname}${cleanUrl.search}${cleanUrl.hash}`);

      let attempts=0;timer=setInterval(()=>{attempts+=1;const searchInput=document.querySelector<HTMLInputElement>('input[placeholder="Search by name or set…"], input[placeholder="Buscar por nombre o set…"]');if(searchInput){const inputSetter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"value")?.set;inputSetter?.call(searchInput,data.name);searchInput.dispatchEvent(new Event("input",{bubbles:true}));searchInput.dispatchEvent(new Event("change",{bubbles:true}));requestAnimationFrame(()=>requestAnimationFrame(finish))}else if(attempts>=40)finish()},100)
    })();
