...
export default function ImportPage() {
  ...
  const text =
    language === "es"
      ? {
          ...
          description:
            "Sube tu CSV de Moxfield. Cada impresión se valida mediante Scryfall ID o set + número de coleccionista antes de guardarse.",
          ...
          reviewHelp:
            "Estas cartas no serán importadas porque su impresión exacta no pudo validarse con Scryfall.",
          csvRow: "Fila CSV",
          set: "Set",
          ...
          supabaseDescription:
            "El importador requiere la configuración pública de Supabase.",
          back: "Volver al panel",
          sellerImport: "Importación del vendedor",
          signInDescription:
            "Inicia sesión para importar cartas a tu inventario.",
          ...
          complete:
            "Importación completa. Ya puedes administrar precios y disponibilidad desde Mi catálogo.",
      ...
      : {
          ...
          description:
            "Upload your Moxfield CSV. Each printing is validated by Scryfall ID or set + collector number before it is saved.",
          ...
          reviewHelp:
            "These cards will not be imported because their exact printing could not be validated with Scryfall.",
          csvRow: "CSV row",
          set: "Set",
          collector: "Collector",
          ...
          supabaseDescription:
            "The importer requires the public Supabase configuration.",
          back: "Back to Dashboard",
          sellerImport: "Seller import",
          signInDescription:
            "Sign in to import cards into your inventory.",
          ...
          complete:
            "Import complete. You can now manage pricing and availability from My Catalog.",
  ...
  async function readFile(file: File) {
    ...
    try {
      ...
      setNotice(
        `${result.filter((row) => row.status === "resolved").length} ${text.validated}`
      );
    } catch (error) {
    ...
    for (const row of resolved) {
      ...
      const { error: inventoryError } = await supabase
        ...
        .upsert(
          ...
          {
            onConflict:
              "seller_id,scryfall_id,language,finish,condition",
          }
        );
      ...
      <main className="grid min-h-screen place-items-center bg-background p-5 text-foreground">
        <section className="w-full max-w-md rounded-3xl border border-border bg-card p-8 text-center">
          ...
          <button
            type="button"
            onClick={signIn}
            className="mt-7 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary font-bold text-primary-foreground"
          >
            <LogIn size={18} />
            {text.continueGoogle}
          </button>
          ...
          <a
            href="/dashboard"
            className="mt-5 inline-block text-sm text-muted-foreground"
          >
            {text.back}
          </a>
        </section>
          ...
          {preview.length > 0 && (
            <div className="mt-6">
              ...
              <button
                ...
                className="mt-4 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-primary font-bold text-primary-foreground disabled:opacity-40"
              >
                <FileUp size={17} />
                {text.importCards} {resolved.length} {text.cards}
              </button>
            </div>
          )}
        </section>
