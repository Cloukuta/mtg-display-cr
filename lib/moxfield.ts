...
export type MoxfieldRow = {
  rowNumber: number; quantity: number; name: string; setCode: string; collectorNumber: string;
  scryfallId: string; condition: string; language: string; finish: string;
};

export type ResolvedCard = MoxfieldRow & {
  status: "resolved" | "unresolved"; error?: string; validFinishes?: string[]; sourceFinish?: string;
  finishDetectedBy?: "moxfield" | "scryfall";
  card?: { scryfall_id: string; name: string; set_code: string; set_name: string; collector_number: string; colors: string[]; rarity: string; image_uri: string | null; };
};

const aliases = {
  quantity: ["quantity", "qty", "count"], name: ["name", "card name", "card"],
  setCode: ["edition", "set", "set code", "setcode"], collectorNumber: ["collector number", "collector #", "collector", "number"],
  scryfallId: ["scryfall id", "scryfall_id"], condition: ["condition"], language: ["language", "lang"], finish: ["finish", "foil"],
} as const;
...
function normalizeLanguage(language: string) {
  ...
  const languages: Record<string, string> = {
    english: "en", en: "en", spanish: "es", español: "es", espanol: "es", es: "es", french: "fr", français: "fr", francais: "fr", fr: "fr",
    german: "de", deutsch: "de", de: "de", italian: "it", italiano: "it", it: "it", portuguese: "pt", português: "pt", portugues: "pt", pt: "pt",
    japanese: "ja", ja: "ja", jp: "ja", korean: "ko", ko: "ko", russian: "ru", ru: "ru", chinese: "zhs", "simplified chinese": "zhs", zhs: "zhs",
    "traditional chinese": "zht", zht: "zht", phyrexian: "ph", ph: "ph",
  };
    ...
    Papa.parse<Record<string, string>>(file, {
      ...
      complete(result) {
        ...
        const rows = result.data.map((record, index) => ({
          rowNumber: index + 2, quantity: Math.max(1, Number.parseInt(value(record, aliases.quantity), 10) || 1),
          name: value(record, aliases.name), setCode: value(record, aliases.setCode).toLowerCase(), collectorNumber: value(record, aliases.collectorNumber),
          scryfallId: value(record, aliases.scryfallId), condition: normalizeCondition(value(record, aliases.condition)),
          language: normalizeLanguage(value(record, aliases.language)), finish: normalizeFinish(value(record, aliases.finish)),
...
function cardImage(card: Record<string, any>) { return card.image_uris?.normal || card.card_faces?.find((face: any) => face.image_uris?.normal)?.image_uris.normal || null; }
function detectSpecialFinish(card: Record<string, any>, sourceFinish: string) {
  const promoTypes = Array.isArray(card.promo_types) ? card.promo_types.map((v: unknown) => String(v).trim().toLowerCase().replace(/[-_\s]/g, "")) : [];
...
export async function resolveWithScryfall(row: MoxfieldRow): Promise<ResolvedCard> {
  ...
  try {
    const response = await fetchScryfallWithRetry(url); const result = await response.json(); const sourceFinish = row.finish;
    const detectedFinish = detectSpecialFinish(result, sourceFinish);
    const validFinishes = Array.isArray(result.finishes) ? Array.from(new Set(result.finishes.map((finish: string) => normalizeFinish(finish)))) as string[] : [];
    if (detectedFinish === "surgefoil" && !validFinishes.includes("surgefoil")) validFinishes.push("surgefoil");
    return { ...row, finish: detectedFinish, sourceFinish, finishDetectedBy: detectedFinish !== sourceFinish ? "scryfall" : "moxfield", status: "resolved", validFinishes,
      card: { scryfall_id: result.id, name: result.name, set_code: result.set, set_name: result.set_name, collector_number: result.collector_number, colors: result.colors || [], rarity: result.rarity, image_uri: cardImage(result) } };
  } catch (error) { return { ...row, status: "unresolved", error: error instanceof Error ? error.message : "Validation failed" }; }
}

export async function resolveBatch(rows: MoxfieldRow[], onProgress?: (done: number) => void) {
  const { resolveBatchWithCollection } = await import("@/lib/moxfield-batch");
  return resolveBatchWithCollection(rows, onProgress);
}
